...
as an Iceberg table backed by a local SQLite catalog.  A verification
step reads every table back via DuckDB's ``iceberg_scan`` to confirm
round-trip fidelity.
...
DEFAULT_CATALOG_DIR = PROJECT_ROOT / "output" / "iceberg"
...
# ── helpers ─────────────────────────────────────────────────────────
def setup_catalog(catalog_dir: Path) -> SqlCatalog:
    """Create (or reconnect to) a PyIceberg SqlCatalog backed by SQLite."""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Export dbt DuckDB tables to Iceberg format",
    )
    parser.add_argument(
        "--db",
        type=Path,
    ...
    parser.add_argument(
        ...
        help=f"Iceberg catalog directory (default: {DEFAULT_CATALOG_DIR})",
    )
    parser.add_argument(
        "--tables",
        nargs="+",
    ...
    logger.info("Iceberg catalog: %s", catalog_dir)
    ...
    for table_name in args.tables:
        ...
        # 3. Verify round-trip
        verified_count = verify_iceberg_table(catalog_dir, table_name)
